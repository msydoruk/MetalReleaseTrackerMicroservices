...
public class AlbumParsedEventConsumerTests : IClassFixture<TestMongoDatabaseFixture>
{
    ...
    public async Task Consume_WhenAlbumsParsed_ShouldSaveDataToMongoWithPendingStatus()
    {
        ...
        _fileStorageServiceMock.SetupSequence(x => x.DownloadFileAsync(It.IsAny<string>()))
        ...
        _fileStorageServiceMock.Verify(x => x.DownloadFileAsync("file1.json"), Times.Once);
        _fileStorageServiceMock.Verify(x => x.DownloadFileAsync("file2.json"), Times.Once);
    }
}
