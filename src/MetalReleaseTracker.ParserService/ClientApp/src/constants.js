export const CATALOGUE_INDEX_STATUS = {
  0: { label: 'New', color: 'info' },
  1: { label: 'Relevant', color: 'success' },
  2: { label: 'Not Relevant', color: 'default' },
  3: { label: 'AI Verified', color: 'success' },
  4: { label: 'Deleted', color: 'error' },
  5: { label: 'AI Rejected', color: 'warning' },
};

export const DISTRIBUTOR_CODES = {
  1: { label: 'Osmose Productions', color: '#e57373' },
  2: { label: 'Drakkar', color: '#64b5f6' },
  3: { label: 'Black Metal Vendor', color: '#81c784' },
  4: { label: 'Black Metal Store', color: '#ffb74d' },
  5: { label: 'Napalm Records', color: '#ba68c8' },
  6: { label: 'Season of Mist', color: '#4dd0e1' },
  7: { label: 'Paragon Records', color: '#ff8a65' },
};

export const MEDIA_TYPES = {
  0: 'CD',
  1: 'LP',
  2: 'Tape',
};

export const CHANGE_TYPE = {
  0: { label: 'New', color: 'info' },
  1: { label: 'Updated', color: 'warning' },
  2: { label: 'Deleted', color: 'error' },
  3: { label: 'Active', color: 'success' },
};

export const PUBLICATION_STATUS = {
  0: { label: 'Unpublished', color: 'warning' },
  1: { label: 'Published', color: 'success' },
};
